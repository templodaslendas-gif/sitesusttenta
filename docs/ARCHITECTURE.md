...
- `embio-product-sections.tsx`: seções independentes `Embio3100Section` e `Embio6000Section`. Grid de áreas (`copy` / `media` / `warning`) equilibra as duas colunas no desktop e lineariza no celular (título → texto → produto → aviso → vídeo → CTA), sem alturas forçadas. O frasco é arte com fundo transparente (`*-frasco.webp`) sobre um "palco" com fundo coerente, base e sombra de contato em CSS — nada de retângulo branco. 3100 em palco claro esverdeado, 6000 em palco azul técnico com halo claro que separa o frasco branco do fundo. Relatos reproduzidos inline e CTA por WhatsApp abaixo do vídeo;
