...
test("defines the approved Embio-first navigation order", async () => {
  ...
  assert.deepEqual(navigationItems.map(({ label }) => label), ["Início", "Embiofert", "Embio 3100", "Embio 6000", "Embio 8000", "TLC Agro", "Sobre Pedro", "Contato"]);
});
...
test("defines the current FAQ questions in the approved commercial hierarchy", async () => {
  ...
  assert.deepEqual(faqItems.map(({ question }) => question), [
    "O que é o Tratamento Embiofert?", "Como Embio 3000 e Propulsor trabalham juntos?", "Qual a diferença entre Embio 3100 e Embio 6000?", "Qual solução considerar quando existe biodigestor?", "Para que serve o Embio 8000?", "Como funciona a linha Ecomax?", "A Susttenta realiza avaliação antes da recomendação?", "Como falar diretamente com Pedro?",
  ]);
});
